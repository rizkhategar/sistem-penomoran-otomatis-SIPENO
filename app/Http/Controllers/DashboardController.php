...
class DashboardController extends Controller
{
    public function index()
    {
        if (auth()->user()->isAdmin()) {
            ...
            $monthly = LetterSubmission::selectRaw('MONTH(COALESCE(submission_date, created_at)) as bulan, COUNT(*) as total')
                ->whereRaw('YEAR(COALESCE(submission_date, created_at)) = ?', [now()->year])
                ->groupBy('bulan')
                ->orderBy('bulan')
                ->pluck('total', 'bulan');
            $monthlyLabels = [];
        ...
        $bidangTotal = LetterSubmission::count();
